...
<?php  
  ...
  if (isset($_GET['les']))  {$les=$_GET['les'];} /* lessons.php?les=?*/
  if (!isset($les)) {$les=1;}     /* lessons.php?les=1 */
$result = mysql_query("SELECT *FROM lessons WHERE id='$les'",$db);
if(!$result)
{
...
<p><b><a href="about.php"> Հեղինակի մասին</a></b></p>
  ...
  <?php 
  echo $myrow['text']; 
  $result = mysql_query("SELECT id,title,description,date,author,view,mini_img FROM data WHERE les='$les'",$db);
if(!$result)
{
echo "<p>Բազայից տվյալների ընտրությունը չի կատարվել:<br></p>";
exit(mysql_error());
