...
$result = mysql_query("SELECT *FROM havelvats WHERE id='$id'",$db);
if(!$result)
{
...
$update=mysql_query("UPDATE havelvats SET view='$new_view' WHERE id='$id'",$db);
}
else
{
...
<br>
  ...
  <?php 
printf("<p class='havel'>%s</p>%s<p class='view'>Դիտումներ: %s</p>",$myrow["title"],$myrow["text"],$myrow["view"]);
  ?>
